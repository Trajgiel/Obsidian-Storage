...
Нельзя определять методы для **алиасов или встроенных типов напрямую**
```go
type AliasInt = int // это просто алиас, не новый тип

func (a AliasInt) Square() int { // ❌ Ошибка: нельзя определить метод для встроенного типа
    return a * a
}
```
