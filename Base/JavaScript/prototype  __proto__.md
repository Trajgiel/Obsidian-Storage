...
`__proto__` - есть у всех объектов и ссылается на `prototype` с помощью которого объект,  `prototype` - это объект, который есть у class, либо fanction (у стрелочных функций его нет), 
